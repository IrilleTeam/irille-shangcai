...
public class OssUtil {
     ...
     * @author yingjianhua
     */
    public static <T extends Entity> String upload(Class<T> beanClass, String filename, File file) {
    ...
    public static <T extends Entity> String upload(Class<T> beanClass, String url) throws IOException {
    	File file = FileDownload.toTemp(url);
    	String filename = MD5Util.getMD5(file)+"."+FileDownload.getSuffix(url);
...
//    	String url = cname==null?("http://"+bucketName+"."+endpoint+"/"+fileKey):("http://"+cname+"/"+fileKey);
    ...
    //取bean的模块名
    public static <T extends Entity> String moduleName(Class<T> beanClass) {
    	String[] s = beanClass.getName().split("\\.");
    	return s[s.length-2];
    }
    //取bean的除去模块名后的剩余部分,并且首字母为小写
    public static <T extends Entity> String beanName(Class<T> beanClass) {
