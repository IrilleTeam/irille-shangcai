...
public class Collection extends Entity {
	...
	public static final Table<Collection> table = TableFactory.entity(Collection.class).column(T.values()).index(true, T.MEMBER, T.WORKS).create();
	...
	public enum T implements IColumnField {
		...
		CREATED_TIME(ColumnTemplate.TIME.showName("收藏时间")),
		;
	...
	private Date createdTime; // 收藏时间 DATETIME(0)
	...
	public Collection init() {
		...
		createdTime = null; // 收藏时间 DATETIME(0)
		return this;
	}
